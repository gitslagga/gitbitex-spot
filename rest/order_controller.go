...
func PlaceOrderService(ctx *gin.Context) {
	...
	if len(req.ClientOid) > 0 {
		_, err = uuid.Parse(req.ClientOid)
		if err != nil {
			mylog.DataLogger.Error().Msgf("[Rest] PlaceOrderService uuid Parse error: %v", err)
	...
	order, err := service.PlaceOrder(GetCurrentAddress(ctx).Id, req.ClientOid, req.ProductId, orderType,
		side, size, price, funds)
	if err != nil {
		mylog.DataLogger.Error().Msgf("[Rest] PlaceOrderService PlaceOrder error: %v", err)
...
func CancelOrderService(ctx *gin.Context) {
	...
	if err != nil {
		mylog.DataLogger.Error().Msgf("[Rest] CancelOrderService GetOrderByClientOid or GetOrderById error: %v", err)
...
func CancelAllOrderService(ctx *gin.Context) {
	...
	if err != nil {
		mylog.DataLogger.Error().Msgf("[Rest] CancelAllOrderService GetOrdersByUserId error: %v", err)
		out.RespCode = EC_NETWORK_ERR
		out.RespDesc = ErrorCodeMessage(EC_NETWORK_ERR)
		ctx.JSON(http.StatusOK, out)
		return
